...
@Component
public class AuthorizationHeaderFilter2
    extends AbstractGatewayFilterFactory<AuthorizationHeaderFilter2.Config> {
  ...
  private Mono<Void> onError(
      ServerWebExchange exchange, String noAuthorizationHear, HttpStatus httpStatus) {
  ...
  private boolean isJwtValid(String jwt) throws Exception {
    String BASE64_SECRET = env.getProperty("token.secret");
    System.out.println("BASE64_SECRET\t" + BASE64_SECRET);
    ...
    try {
      ...
      // Parse the JWT
      Jwts.parser().setSigningKey(secretKey).build().parseClaimsJws(jwt);
